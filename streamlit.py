...
st.write('# Preditor câncer') # df, err, func, keras!
...
def get_user_date():
    ...
    consdiag = col2.number_input('Diferença entre consulta e diagnóstico (dias)', value=25, format='%i')
    tratcons = col2.number_input('Diferença entre consulta e tratamento (dias)', value=25, format='%i')
    diagtrat = col2.number_input('Diferença entre diagnóstico e tratamento (dias)', value=25, format='%i')
    anodiag = col2.slider('Ano do diagnóstico', 1999, 2022, 2010)
    drs = col2.slider('DRS', 1, 17, 10)
    rras = col2.slider('RRAS', 1, 17, 7)
    ...
    user_data = {'Column2': 'example_value', 'IDADE': idade, 'SEXO': sexo, 'IBGE': ibge, 'CATEATEND': catatend,
    'DIAGPREV': diagprev, 'EC': ec, 'TRATHOSP': trathosp, 'NENHUM': nenhum, 'CIRURGIA': cirur, 'RADIO': radio,
    'QUIMIO': quimio, 'HORMONIO': horm, 'TMO': tmo,
    'IMUNO': imuno, 'OUTROS': outros, 'NENHUMANT': nenhumant, 'CONSDIAG': consdiag, 'TRATCONS': tratcons, 'DIAGTRAT': diagtrat, 'ANODIAG': anodiag, 'DRS': drs, 'RRAS': rras, 'RECNENHUM': recnenhum, 'IBGEATEN': ibgeaten, 'ESCOLARI_2': escolari
    }
    
    return user_data

# Função de POST para os modelos
def prediction(data, url):
...
if col3.button('Realizar Predição do modelo'):
    result = json.loads(prediction(user_input_variables, url))
    col3.metric('Vivo ano 1', result['result'][0])
